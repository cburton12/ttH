...
// Dearest and most respected user //guide
...
void ttHMultileptonLooseEventSaver::initialize(std::shared_ptr<top::TopConfig> config, TFile* file, const std::vector<std::string>& extraBranches) {
  ...
  for (auto systematicTree : m_treeManagers){
    ...
    if(!m_doSystematics) {
      ...
      //========================= Jpsi =========================
      std::vector<VectorWrapper*> Jpsivec; //cdb

      // passed Jpsi
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (char) DiMuon.auxdata<char>("passed_Jpsi"); },  *systematicTree, "Jpsi_passed_Jpsi");
      // mass and mass error
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_mass"); },  *systematicTree, "Jpsi_mass");
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_massErr"); },  *systematicTree, "Jpsi_massErr");
      // invariant mass info
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_TauInvMassPVMaxSumPt2"); },  *systematicTree, "Jpsi_TauInvMassPVSumPt2");
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_TauInvMassPVMinA0"); },  *systematicTree, "Jpsi_TauInvMassPVMinA0");
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_TauInvMassPVMinZ0"); },  *systematicTree, "Jpsi_TauInvMassPVMinZ0");
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_TauErrInvMassPVMaxSumPt2"); },  *systematicTree, "Jpsi_TauErrInvMassPVMax");
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_TauErrInvMassPVMinA0"); },  *systematicTree, "Jpsi_TauErrInvMassPVMinA0");
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.auxdata<float>("Jpsi_TauErrInvMassPVMinZ0"); },  *systematicTree, "Jpsi_TauErrInvMassPVMinZ0");
      // chiSquared
      Wrap2(Jpsivec, [](const xAOD::Vertex& DiMuon){ return (float) DiMuon.chiSquared(); },  *systematicTree, "Jpsi_chiSquared");

      vec_Jpsi_wrappers.push_back(VectorWrapperCollection(Jpsivec));
...
void ttHMultileptonLooseEventSaver::saveEvent(const top::Event& event){
  ...
  vec_tau_wrappers[event.m_ttreeIndex].push_all(event.m_tauJets);
    ...
    ////save ALL jets
    //xAOD::JetContainer* calibratedJets(nullptr);
    //top::check(evtStore()->retrieve(calibratedJets, m_config->sgKeyJetsTDS(sysHash,false)), "Failed to retrieve calibrated jets"); //cdb
    
  //save ALL onia candidates
  const xAOD::VertexContainer* DiMuonVertices(nullptr);
  top::check(evtStore()->retrieve(DiMuonVertices,"BPHY9OniaCandidates"), "Failed to retrieve dimuon vertices"); //cdb
    
  vec_Jpsi_wrappers[event.m_ttreeIndex].push_all(*DiMuonVertices);
